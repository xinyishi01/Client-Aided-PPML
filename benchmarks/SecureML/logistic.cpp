...
int main(int argc, char** argv){	
	...
	Mat x_batch(B,D), tx_batch,txa_batch, y_batch(B,1), xa_batch(B,D), a_batch(B, D);
	...
	//	cerr << "it: " << i << endl;
		...
		if(party == ALICE){
			inner = compute_inner0(W,x_batch, xa_batch, b_IT1[i],c_IT1[i], W1b );
			d0 = logistic(inner, y_batch, party);
			d0_buf = d0-b_IT2[i];
			//reconstruct(d0,party);
		}
		else{
			inner = compute_inner1(W,x_batch, xa_batch, b_IT1[i],c_IT1[i], W0b );
		...
		if(party == ALICE){
			update_W0(W,tx_batch,txa_batch, b_IT2[i],d0, c_IT2[i], d1_buf); 
		}
		else{
			update_W1(W,tx_batch, txa_batch, b_IT2[i],d1, c_IT2[i], d0_buf);
		}
