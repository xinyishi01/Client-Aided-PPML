...
/*
void send_mat_arr(Mat *x, int l, NetIO * io) {
    vector<unsigned long int> temp(x.cols()*x.rows());
    for(int j=0; j<x.rows(); j++) {
        for(int k=0; k<x.cols(); k++) {
            temp[j*x.cols()+k] = x(j,k);
...
void send_mat(Mat& x, NetIO *io, double& comm, double& comp, Time_Calculater& mytime) {
