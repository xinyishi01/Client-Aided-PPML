...
Mat compute_delta0(Mat& W0,Mat& x0, Mat& y0, Mat& e, Mat& b0,Mat& c0, Mat& Wb1 ){
	...
	for (int i = 0; i < B; i++)
		f.col(i) = f.col(i) + W0; 
	d = spe_product(x0, f)+e*W0+c0-y0*(unsigned long int)(1<<(L+8));
	return d;
...
Mat compute_delta1(Mat& W1,Mat& x1, Mat& y1, Mat& e, Mat& b1,Mat& c1, Mat& Wb0 ){
	...
	d = spe_product(x1, f)+ spe_product(e, f_)+c1-y1*(unsigned long int)(1<<(L+8));
	return d;
...
int main(int argc, char** argv){
	...
	//	cerr << i << ' ' << "after\n";
		...
		cout << "a*b_IT1 == c_IT1: " <<(spe_product(reconstruct(a_batch,party), reconstruct(b_IT1,party)) == reconstruct(c_IT1,party))<<endl;
		*/
	...
	//	cerr << "1.5\n";
		...
		if(party == ALICE){
			d = compute_delta0(W,x_batch, y_batch, xa_batch, b_IT1,c_IT1, W1b );
			d0_buf = d-b_IT2;
			//reconstruct(d0,party);
		}
		else{
			d = compute_delta1(W,x_batch, y_batch, xa_batch, b_IT1,c_IT1, W0b );
			d1_buf = d-b_IT2;
	...
	//	cerr << "2\n";
		...
		if(party == ALICE){
			update_W0(W,tx_batch,txa_batch, b_IT2,d, c_IT2, d1_buf); 
		}
		else{
			update_W1(W,tx_batch, txa_batch, b_IT2,d, c_IT2, d0_buf);
		}
