...
/*------------------------------------------------------------------------*/
	...
	void receive_encodings(Mat &counts) {
		...
		uint8_t* enc0 = new uint8_t[r * c * P * 16];
		uint8_t* enc1 = new uint8_t[r * c * P * 16];
		//cout << "receiving encodings ...\n";
		io_alice->recv_data(&enc0[0], sizeof(uint8_t) * r * c * P * 16);
		io_bob->recv_data(&enc1[0], sizeof(uint8_t) * r * c * P * 16);
	...
	void check_malicious_activation_function_client(int dN) {
		int dM = dN * dk + dk;
		Mat counts(dM, 1);
		Mat tau(dM, 1);
		receive_encodings(counts);
		Mat temp_z(dM, 1);
		add_mat(prf_alice_same, prf_bob_same, temp_z);
		tau = counts.cwiseProduct(temp_z);
		distribute_mat_same(counts, prf_alice_same, prf_bob_same, io_alice, io_bob);
		distribute_mat_same(tau, prf_alice_same, prf_bob_same, io_alice, io_bob);
	}
	...
	void update() {
		/*--------------------------Malicious--------------------------*/
		if(id != "L3") dNum += B * D_out * 2;
		else {
			dNum += B * D_out + B * K * (L + 1);
			//cout << "dNum = " << dNum << endl;
		}
		
		if(dNum >= dNN) {
			//cout << "checking malicious activation function...\n";
			check_malicious_activation_function_client(dNum);
			dNum = 0;
		}
		/*--------------------------Malicious--------------------------*/
	}
